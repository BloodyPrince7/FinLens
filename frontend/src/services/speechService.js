...
/**
 * Plays high-definition speech audio generated by the backend Google TTS engine.
 * Guaranteed to work on all devices, even when Windows does not have a Hindi voice installed.
 *
 * @param {string} text
 * @param {'en' | 'hi'} [language='hi']
 * @returns {Promise<void>}
 */
export function playServerTts(text, language = 'hi') {
  return new Promise(async (resolve) => {
    ...
    try {
      ...
      console.warn('[TTS] Server audio failed, falling back to local speech synthesis:', err)
      speakLocal(text, language).then(resolve).catch(resolve)
    }
  })
}

function speakLocal(cleanText, language) {
...
/**
 * Reads text aloud. For Hindi, if the client system lacks a Hindi voice pack,
 * automatically routes through the backend Google Hindi TTS engine for crystal-clear readout.
 *
 * @param {string} text
 * @param {'en' | 'hi'} language
 * @returns {Promise<void>}
...
export function speak(text, language = 'en') {
  ...
  const hasBrowserHindiVoice = voices.some(
    (v) => v.lang?.toLowerCase().startsWith('hi') || /hindi|हिन्दी/i.test(v.name)
  )

  if (isHindi && !hasBrowserHindiVoice) {
    return playServerTts(cleanText, 'hi')
  }

  return speakLocal(cleanText, language)
}
...
export function isSpeechActive() {
  return Boolean(currentSpeechResolver || currentAudio)
}
